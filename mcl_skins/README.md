...
mcl_skins.register_item({
    type = "hair",
    texture = "my_hair.png",
    mask = "my_hair_mask.png",
    preview_rotation = {x = 0, y = 0},
    rank = 70,
})
```
...
- **texture**: A texture string that can be used in the node definition.
...
for _, skin in ipairs(skins) do
    print("Skin ID: " .. skin.id)
    print("Texture: " .. skin.texture)
    print("Slim arms: " .. tostring(skin.slim_arms))
end
```

### mcl_skins.get_node_id_by_player(player)

**player** is a player ObjectRef. Returns a string node ID based on player's current skin for use by mods that want to register nodes that use the player skin.
...
```lua
local color = mcl_skins.base_color[1]  -- "#fce0d0" (Light skin)
```
...
```lua
local color = mcl_skins.color[4]  -- "#ff0000" (Red)
```
...
local texture_string = mcl_skins.compile_skin(skin_data)
```
...
To create custom skins for this mod, you can create a separate mod that depends on `mcl_skins` and registers custom skin items:
...
Players can use the `/skin` command to open the skin customization interface. From there, they can:

1. Select different tabs for different body parts and clothing
2. Choose from available items in each category
3. Customize colors for colorable items (base, hair, clothing)
4. Preview their changes
5. Changes are automatically saved

## Notes

- If you need to create sub-tasks, you can create issues
- This mod is compatible with both VoxeLibre and standard Minetest Game
- Skin data is saved automatically when players leave the server
- Custom skin mods can be created separately and will integrate with this system
